...
const state = { accessStep: 0, authed: false, route: 'home', exploreView: 'life', selectedCollection: null, selectedSubtopic:null, selectedSubtopicLabel:null, selectedItem:null, selectedAma:null, selectedEvent:null, selectedActivity:null, selectedResource:null, selectedMember:null, memberProfileTab:'posts', memberProfileOrigin:'home', selectedHelpRequest:null, detailOrigin:'home', resourceOrigin:'explore', searchOrigin:'home', scrollPositions:{home:0,explore:0,search:0,collection:0,connect:0,events:0,homeCollection:0,resourcesLibrary:0}, search: '', sort: 'latest', homeTab:'all', homeSearch:'', homeVisible:4, homeTopicsVisited:false, homeRecommendationActive:false, homeFallbackShown:false, homeFallbackActive:false, homeCollection:null, homeCollectionQuery:'', homeCollectionFilter:'All', homeCollectionSort:'latest', resourceSearch:'', resourceSource:'All', resourceFormat:'All formats', resourceSort:'latest', resourceVisible:4, resourceEventFilter:null, resourceDiscussionContext:'', attendedEvents:new Set(), likedPosts:new Set(), likedResources:new Set(), commentedPosts:new Set(), authoredPosts:new Set(), seenUpdates:new Set(), homeQuickFilter:'All', homeDetailedFilters:new Set(), homeFilterDraft:new Set(), collectionQuickFilter:'All', collectionDetailedFilters:new Set(), collectionFilterDraft:new Set(), collectionSort:'latest', searchQuickFilter:'All', searchDetailedFilters:new Set(), searchFilterDraft:new Set(), searchSort:'latest', eventQuickFilter:'All', eventDetailedFilters:new Set(), eventFilterDraft:new Set(), eventSort:'upcoming', eventChosenDate:'2026-10-12', eventPlansExpanded:false, eventUpcomingExpanded:false, eventPastExpanded:false, eventReturnFlow:null, eventReturnDialog:false, eventScrollTarget:null, cancelledRegistrations:new Set(), filterPanelScope:null, saved: new Set(), savedResources:new Set(), recentSearches: ['school support', 'travelling'], createTitle:'', createBody:'', createTopics:new Set(), createStep: 1, profileTab: 'saved', profileSavedTab:'all', commentSort: 'helpful', modFilter: 'Open reports', eventView:'upcoming', eventType:'All', registered:new Set(), joinedActivities:new Set(['east-coast-walk']), connectView:'community', connectFilter:'All', meetupStep:1, meetupType:'Meetup', helpStep:1, helpStatus:'open', volunteerOffer:false, helpServiceAvailable:true, onboarding:{firstName:'',lastName:'',phone:'',otp:['','','','','',''],username:'SunlitKoi',avatar:'avatar-style-1',relationship:'',ageRange:'',lifeStage:'',topics:new Set(),otherTopic:'',resendAvailableAt:0} };
state.createStages = new Set();
let viewedMemberProfile = null;
const app = document.querySelector('#app');
let overlayReturnSelector=null;
let resendTimer=null;
function memberName(){return state.onboarding.username.trim()||'SunlitKoi';}
function initialsFromUsername(value){const readable=(value||'').trim().replace(/([a-z0-9])([A-Z])/g,'$1 $2');const parts=readable.split(/[^A-Za-z0-9]+/).filter(Boolean);if(parts.length>1)return parts.slice(0,2).map(part=>part[0]).join('').toUpperCase();return (parts[0]||'SK').slice(0,2).toUpperCase();}
function memberInitials(){return initialsFromUsername(memberName());}
const avatarStyles=['avatar-style-1','avatar-style-2','avatar-style-3','avatar-style-4','avatar-style-5'];
function memberAvatarStyle(){return avatarStyles.includes(state.onboarding.avatar)?state.onboarding.avatar:'avatar-style-1';}
function memberAvatar(){return memberInitials();}
function avatarOptionsMarkup(selected=memberAvatarStyle()){const initials=accessValue(memberInitials());return avatarStyles.map((value,index)=>`<button type="button" data-focus-key="avatar-${value}" class="avatar-option ${value} ${selected===value?'selected':''}" aria-label="Avatar colour ${index+1}" aria-pressed="${selected===value}" onclick="setAccessAvatar('${value}')">${initials}</button>`).join('');}
function updateAvatarInitials(value){state.onboarding.username=value;const initials=initialsFromUsername(value);document.querySelectorAll('.avatar-option').forEach(option=>{option.textContent=initials;});}
// ---------- API client ----------
...
function applyMember(member) {
  ...
  state.onboarding.firstName = member.firstName || '';
  state.onboarding.lastName = member.lastName || '';
...
async function loadBootstrap() {
  ...
  state.likedResources = new Set(data.likedResources || []);
...
function go(route, detail = null) {
  const detailRoutes=['detail','eventDetail','amaDetail','meetupDetail','resourceDetail','memberProfile'];
  ...
  } else if(route==='memberProfile'){
    if(detail!==null){state.selectedMember=detail;state.memberProfileTab='posts';if(state.route!=='memberProfile')state.memberProfileOrigin=state.route;}
...
function shell(contentHtml, active = state.route) {
  ...
  const mobileTitles={home:'Haemily',homeCollection:state.homeCollection||'Community posts',explore:'Resources',resourcesLibrary:'All resources',resourceDetail:'Resource details',detail:'Post details',memberProfile:state.selectedMember||'Member profile',create:'Create post',profile:'Profile',events:'Events',eventDetail:'Event details'};
  ...
  const detailRoute=['detail','eventDetail','resourceDetail','memberProfile'].includes(state.route);
  ...
  const detailBack=state.route==='resourceDetail'?'backFromResource()':state.route==='memberProfile'?'backFromMemberProfile()':'backFromDetail()';
...
function accessProgress(){if(state.accessStep<3)return `<div class="access-setup-context">${icons.lock}<span><strong>Account setup</strong><small>Verify access and create your private profile</small></span></div>`;const complete=state.accessStep===6,current=complete?3:state.accessStep-2,label=complete?'Personalisation complete':`Question ${current} of 3`,context=complete?'Ready for Home':'';return `<div class="access-progress-copy"><span>${label}</span>${context?`<span>${context}</span>`:''}</div><div class="progress" aria-label="${label}">${Array.from({length:3},(_,i)=>`<span class="progress-step ${complete||i<current?'done':''}"></span>`).join('')}</div>`;}
function accessReminder(copy=''){return copy?`<p class="access-reminder">${icons.settings}<span>${copy}</span></p>`:'';}
...
function accessTopBack(step){return `<button type="button" class="button button-ghost access-back access-back-top" onclick="goAccessStep(${step})">${icons.back} Back</button>`;}
function resetAccessScroll(){window.scrollTo(0,0);requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}));}
...
function accessScreen() {
  ...
  if(state.accessStep===5) body=`${progress}${accessTopBack(['caregiver','both'].includes(answer.relationship)?4:3)}<h1>What would you like to find on Haemily?</h1><p class="lede">Choose as many topics as you like. You can change these later in Preferences.</p><div class="onboarding-topic-grid" aria-label="Topics">${topicOptions.map(([value,icon])=>`<button type="button" data-focus-key="topic-${accessValue(value)}" class="onboarding-choice-card topic-choice ${answer.topics.has(value)?'selected':''}" aria-pressed="${answer.topics.has(value)}" onclick="toggleOnboardingTopic('${value.replace(/'/g,"\\'")}')">${onboardingArt(icon)}<strong>${value}</strong></button>`).join('')}<button type="button" data-focus-key="topic-Other" class="onboarding-choice-card topic-choice other-choice ${answer.topics.has('Other')?'selected':''}" aria-pressed="${answer.topics.has('Other')}" onclick="toggleOnboardingTopic('Other')">${onboardingArt('plus')}<strong>Other</strong></button></div>${answer.topics.has('Other')?`<div class="field other-topic-field"><label for="other-topic">Other topic</label><input class="input" id="other-topic" maxlength="50" value="${accessValue(answer.otherTopic)}" placeholder="Type a topic" oninput="state.onboarding.otherTopic=this.value"/></div>`:''}<div class="access-actions"><button class="button button-ghost access-skip" type="button" onclick="skipTopics()">Skip for now</button><button class="button button-primary" type="button" onclick="finishPersonalisation()">Continue</button></div>${accessReminder()}`;
  if(state.accessStep===0)body=accountAccessBody;
  if(state.accessStep===6)body=`<div class="success-brand" aria-label="Haemily"><img src="./assets/haemily-logo.png" alt=""/><strong><span class="success-typing" aria-hidden="true">Haemily</span></strong></div><div class="success-copy"><h1>You’re all set!</h1><p class="lede">You can now continue to Haemily.</p></div><button class="button button-primary completion-action" type="button" onclick="enterHome()">Explore now</button>`;
  return `<main class="access-shell access-step-${state.accessStep}"><div class="access-brand ${state.accessStep<=5?'with-name':''}"><div class="brand"><img class="brand-logo" src="./assets/haemily-logo.png" alt=""/>${state.accessStep<=5?'<strong class="access-brand-name">Haemily</strong>':''}</div></div><div class="access-main"><section class="access-card">${body}</section></div></main>`;
}

function goAccessStep(step){clearInterval(resendTimer);state.accessStep=step;render();resetAccessScroll();requestAnimationFrame(()=>{if(step===1){if(window.matchMedia('(min-width: 768px)').matches)document.querySelector('.otp-input')?.focus();startResendCountdown();}});}
async function submitPhone(e){
  e.preventDefault();
  const firstInput=document.querySelector('#first-name'),lastInput=document.querySelector('#last-name'),input=document.querySelector('#phone');
  const firstName=firstInput.value.trim(),lastName=lastInput.value.trim(),digits=input.value.replace(/\D/g,'');
  const firstOk=!!firstName,lastOk=!!lastName,phoneOk=digits.length===8,ok=firstOk&&lastOk&&phoneOk;
  document.querySelector('#first-name-error').classList.toggle('show',!firstOk);
  document.querySelector('#last-name-error').classList.toggle('show',!lastOk);
  const errorEl=document.querySelector('#phone-error');
  errorEl.textContent='Enter a valid eight-digit Singapore mobile number.';
  errorEl.classList.toggle('show',!phoneOk);
  firstInput.setAttribute('aria-invalid',String(!firstOk));
  lastInput.setAttribute('aria-invalid',String(!lastOk));
  input.setAttribute('aria-invalid',String(!phoneOk));
  if(!ok)return;
  state.onboarding.firstName=firstName;
  state.onboarding.lastName=lastName;
...
function skipAgeRange(){state.onboarding.ageRange='';state.onboarding.lifeStage='';goAccessStep(5);}
...
// ---------- "Me" badge and member-profile links ----------
// Uses flags the API already computed (isMine/authorUsername) instead of
// scanning rendered DOM text for name matches.
function meTag(){return '<span class="me-tag">Me</span>';}
function isOwnPost(item){return !!item.isMine;}
...
function postAuthorMarkup(item){
  ...
  if(item.authorUsername)return memberProfileLink(item.authorUsername,item.author);
  return accessValue(item.author);
}
async function openMemberProfile(username){
  try{
    const profile=await api('GET',`/members/${encodeURIComponent(username)}`);
    viewedMemberProfile=profile;
    go('memberProfile',username);
  }catch(err){
    toast('This member’s profile could not be opened');
  }
}
function backFromMemberProfile(){go(state.memberProfileOrigin||'home');}

function contentCard(item, compact=false) {
  return `<article class="card content-card ${compact?'compact-content-card':''}">
    <div class="card-top"><div class="author"><span class="author-avatar ${item.org?'org':''}">${isOwnPost(item)?memberAvatar():item.initials}</span><div><div class="author-name">${postAuthorMarkup(item)}</div><div class="meta"><span>${item.date}</span></div></div></div></div>
...
function homePostRow(item,showImage=true){const saved=state.saved.has(item.id);return `<article class="community-post-row browse-row"><a class="post-row-main post-row-link browse-row-primary" href="#/detail/${encodeURIComponent(item.id)}" onclick="event.preventDefault();go('detail','${item.id}')"><div class="post-row-meta"><span class="topic-tag">${postTopic(item)}</span><strong>${postAuthorMarkup(item)}</strong><span>· ${relativePostTime(item)}</span></div><h3>${item.title}</h3><p>${item.excerpt}</p>${showImage?postImagePreview(item):''}</a><div class="post-row-actions"><span>${icons.heart} ${item.reactions} likes</span><span>${icons.message} ${item.comments} comments</span><button class="save-button browse-row-action ${saved?'saved':''}" aria-label="${saved?'Remove '+item.title+' from saved posts':'Save '+item.title}" onclick="toggleSave('${item.id}',this)">${saved?icons.bookmarkFilled:icons.bookmark}<span>${saved?'Saved':'Save'}</span></button></div></article>`;}
function homeCollectionCard(item,carouselId){return `<a class="home-topic-post" href="#/detail/${encodeURIComponent(item.id)}" onclick="event.preventDefault();openCarouselPost('${item.id}')"><span class="topic-tag">${postTopic(item)}</span><h3>${item.title}</h3><p>${item.excerpt}</p><div><span>${postAuthorMarkup(item)} · ${relativePostTime(item)}</span><span class="home-topic-engagement"><span>${icons.heart} ${item.reactions} likes</span><span>${icons.message} ${item.comments} comments</span></span></div></a>`;}
...
function createScreen(){
  ...
  const body=`<section class="composer-section composer-write-section"><div class="field"><label for="post-title">Title</label><input class="input" id="post-title" maxlength="120" placeholder="Give your post a clear title" value="${accessValue(title)}" oninput="state.createTitle=this.value;document.querySelector('#post-preview-title').textContent=this.value||'Your post title';"/></div><div class="field composer-body-field"><label for="post-body">Your post</label><div class="composer-editor">${formattingToolbar}<textarea class="textarea" id="post-body" placeholder="What would you like to share?" oninput="state.createBody=this.value;document.querySelector('#post-preview-copy').textContent=this.value||'Your post content will appear here.'">${accessValue(draftBody)}</textarea></div><p class="field-hint">Do not include children’s names, diagnoses or other identifying information.</p></div><div class="field"><label>Topic</label><div class="chips" role="group" aria-label="Topics">${topics.map(([name])=>`<button type="button" class="chip ${state.createTopics.has(name)?'selected':''}" aria-pressed="${state.createTopics.has(name)}" onclick="toggleCreateChip('createTopics','${name.replace(/'/g,"\\'")}')">${name}</button>`).join('')}</div></div><div class="field"><label>Life stage</label><div class="chips" role="group" aria-label="Life stages">${lifeStages.map(([name])=>`<button type="button" class="chip ${state.createStages.has(name)?'selected':''}" aria-pressed="${state.createStages.has(name)}" onclick="toggleCreateChip('createStages','${name.replace(/'/g,"\\'")}')">${name}</button>`).join('')}</div></div></section><section class="composer-section post-review-section"><div class="composer-section-heading"><div><h2 class="composer-section-title">Preview</h2><p class="small muted">This is how your post will appear to the community.</p></div></div><article class="post-review-preview"><div class="post-review-meta"><span class="topic-tag">${accessValue(previewTag)}</span><span class="post-review-author">${meTag()}<strong>${accessValue(memberName())}</strong><span>· Just now</span></span></div><div class="post-review-content"><span id="post-preview-title">${title||'Your post title'}</span><p id="post-preview-copy">${draftBody||'Your post content will appear here.'}</p></div><div class="post-review-footer"><div><span>${icons.heart} 0 likes</span><span>${icons.message} 0 comments</span></div><span class="post-review-save">${icons.bookmark} Save</span></div></article></section><div class="composer-publish-bar"><button class="button button-primary" onclick="publishPost(this)">Post to community</button></div>`;
...
// Desktop layout fix carried over from the design: moves the author block,
// published date and topic tags above the h1 in the DOM (matches the
// two-column desktop reading layout), and folds the date into the author
// line. Uses the API's authorUsername/isMine flags rather than DOM text
// matching.
function applyPostDetailHeaderOrder(root=app){
  const header=root.querySelector('.content-detail-page .detail-header');
  if(!header)return;
  const item=content.find(entry=>entry.id===state.selectedItem);
  const title=header.querySelector('h1');
  const author=header.querySelector('.detail-author');
  const dates=header.querySelector('.detail-dates');
  const tags=header.querySelector('.metadata-line');
  if(!title||!author)return;
  const publishedDate=dates?.querySelector('span:first-child');
  if(item&&publishedDate){publishedDate.className='detail-publish-date';publishedDate.textContent=`${item.date} 2026`;author.appendChild(publishedDate);}
  if(dates&&!dates.children.length)dates.remove();
  [author,dates?.children.length?dates:null,tags].filter(Boolean).forEach(section=>header.insertBefore(section,title));
}
...
function renderResourceDetailEngagementRow(resource){
  const discussion=document.querySelector('.resource-detail-page .resource-discussion');
  if(!discussion)return;
  const liked=!!resource.liked;
  const baseLikes=(resource.likes||0)-(liked?1:0);
  discussion.insertAdjacentHTML('beforebegin',`<div class="detail-action-row resource-detail-engagement"><button class="detail-row-action ${liked?'active':''}" data-base-likes="${baseLikes}" onclick="toggleResourceLike('${resource.id}',this)" aria-label="Like this resource">${icons.heart}<span>${resource.likes||0} likes</span></button><button class="detail-row-action" onclick="document.querySelector('.resource-discussion')?.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'})" aria-label="View comments">${icons.message}<span>Comments</span></button></div>`);
}
async function toggleResourceLike(id,button){
  const active=button.classList.toggle('active');
  const base=Number(button.dataset.baseLikes||0);
  button.querySelector('span').textContent=`${base+(active?1:0)} likes`;
  try{
    if(active){await api('POST',`/resources/${encodeURIComponent(id)}/likes`);state.likedResources.add(id);}
    else{await api('DELETE',`/resources/${encodeURIComponent(id)}/likes`);state.likedResources.delete(id);}
  }catch(err){
    button.classList.toggle('active');
    const rolledBack=button.classList.contains('active');
    button.querySelector('span').textContent=`${base+(rolledBack?1:0)} likes`;
...
function resourceDetailScreen(){
  ...
  queueMicrotask(()=>{loadCommentsInto('resource',resource.id);renderResourceDetailEngagementRow(resource);});
...
function publicMemberCommentRow(comment){
  return `<article class="profile-comment-row public-member-comment"><div class="post-row-meta"><span>· ${accessValue(comment.ageLabel)}</span></div><p class="comment-text">${accessValue(comment.text)}</p>${comment.postId?`<p class="comment-origin">On “${accessValue(comment.postTitle)}”</p><button class="button button-ghost public-member-thread-link" onclick="go('detail','${comment.postId}')"><span>View thread</span>${icons.arrow}</button>`:''}</article>`;
}
function memberProfileScreen(){
  const profile=viewedMemberProfile;
  if(!profile||profile.username!==state.selectedMember){backFromMemberProfile();return '';}
  setTimeout(ensureActiveProfileTab,0);
  const active=state.memberProfileTab==='comments'?'comments':'posts';
  const rows=active==='posts'?profile.posts.map(item=>profilePostRow(item)):profile.comments.map(publicMemberCommentRow);
...
function applyDesktopHomeCreateFab(root=app){
  if(state.route!=='home')return;
  root.insertAdjacentHTML('beforeend',`<button class="desktop-home-create-fab" onclick="go('create')">${icons.plus}<span>Create post</span></button>`);
}
function applyCommentSortControls(root=app){
  root.querySelectorAll('.comment-sort-control').forEach(control=>{
    const select=control.querySelector('.sort-select');
    if(!select||control.querySelector('.comment-sort-value'))return;
    const value=document.createElement('span');
    value.className='comment-sort-value';
    value.textContent=select.options[select.selectedIndex]?.text||'';
    select.insertAdjacentElement('afterend',value);
    value.insertAdjacentHTML('afterend',icons.chevronDown);
    select.addEventListener('change',()=>{value.textContent=select.options[select.selectedIndex]?.text||'';});
  });
}
...
function render(){
  ...
  const routes={home:homeScreen,homeCollection:homeCollectionScreen,explore:exploreScreen,resourcesLibrary:resourcesLibraryScreen,resourceDetail:resourceDetailScreen,detail:detailScreen,memberProfile:memberProfileScreen,create:createScreen,profile:profileScreen,events:eventsScreen,eventDetail:eventDetailScreen};
  app.innerHTML=(routes[state.route]||homeScreen)();applyCommentSortControls();applyDesktopHomeCreateFab();document.body.classList.toggle('has-fab',!!app.querySelector('.mobile-fab'));document.body.classList.toggle('detail-view',['detail','eventDetail','resourceDetail','memberProfile'].includes(state.route));document.body.classList.toggle('create-view',state.route==='create');resetL1Topbar();if(state.route==='home')requestAnimationFrame(()=>{initHomeCarousels();if(state.homeTab==='all')initHomeLazyLoad();});if(state.route==='resourcesLibrary')requestAnimationFrame(initResourceLazyLoad);if(state.route==='eventDetail'&&state.eventScrollTarget==='resources'){state.eventScrollTarget=null;setTimeout(()=>document.querySelector('#event-resources')?.scrollIntoView({block:'start'}),30);} const titles={home:'Home',homeCollection:state.homeCollection||'Community posts',explore:'Resources',resourcesLibrary:'All resources',resourceDetail:'Resource',eventDetail:'Event',profile:'Profile',memberProfile:state.selectedMember||'Member profile'}; document.title=`${titles[state.route]||state.route[0].toUpperCase()+state.route.slice(1)} · Haemily`;
}
