...
async function wipe() {
  await db.query(`TRUNCATE TABLE comment_likes, comments, reactions, saves, event_registrations, sessions, posts, events, resources, members RESTART IDENTITY CASCADE`);
}
...
async function seedEventsAndResources() {
  for (const event of events) {
    await db.query(`INSERT INTO events (id, data) VALUES ($1,$2) ON CONFLICT (id) DO NOTHING`, [event.id, JSON.stringify(event)]);
  }
  for (const resource of resources) {
    await db.query(`INSERT INTO resources (id, data) VALUES ($1,$2) ON CONFLICT (id) DO NOTHING`, [resource.id, JSON.stringify(resource)]);
  }
}
...
async function seedActivity() {
  ...
  await db.query(`INSERT INTO reactions (member_id, post_id, kind) VALUES ($1,'first-trip','helpful'),($1,'after-diagnosis','helpful') ON CONFLICT DO NOTHING`, ['member-brightkite']);
  await db.query(`INSERT INTO saves (member_id, item_type, item_id) VALUES ($1,'post','teachers'),($1,'resource','travel-session-recording') ON CONFLICT DO NOTHING`, ['member-brightkite']);
  await db.query(
